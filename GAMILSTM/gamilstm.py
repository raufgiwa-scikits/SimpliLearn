...
class GAMINet(tf.keras.Model):
    ...
    def __init__(self, meta_info,
        ...
        self.early_stop_main = early_stop_thres[0]
        self.early_stop_interaction = early_stop_thres[1]
        self.early_stop_fine = early_stop_thres[2]
    ...
    def Fit_Main_Effect(self, Tr_x, Tr_y, sample_weight=None, VAL_X=[], VAL_Y=[], VAL_W=[]):

        last_improvement = 0
        violations=0
        best_validation = np.inf*np.array([1]*len(VAL_X))
        current_validation = np.inf*np.array([1]*len(VAL_X))
        ...
        for epoch in range(self.main_effect_epochs):
            ...
            self.err_train_main_effect_training.append(self.evaluate(tr_x, tr_y, tr_sw,  main_effect_training=False, interaction_training=False))  # self.err_val_main_effect_training.append(self.evaluate(val_x, val_y, sample_weight[self.val_idx],  main_effect_training=False, interaction_training=False))
            
            err_valid_list=[]
            for i in range(len(VAL_X)):
                val_x, val_y, val_w = VAL_X[i], VAL_Y[i], VAL_W[i]
                err_valid_list.append(self.evaluate(val_x, val_y, val_w,  main_effect_training=False, interaction_training=False))                
            current_validation=np.array(err_valid_list)                
            self.err_val_main_effect_training.append(current_validation)     
            msg =f"Main effects training epoch: {epoch + 1}, train loss: {self.err_train_main_effect_training[-1]}, val loss: {self.err_val_main_effect_training[-1]}"
            fprint(2,self.message_list, msg)
            violations=violations+np.sum(np.sum(current_validation-best_validation>0)>0)
            best_validation=current_validation
            # if self.err_val_main_effect_training[-1] < best_validation:
            #     best_validation = self.err_val_main_effect_training[-1]
            #     last_improvement = epoch
            
            if (violations > self.early_stop_main) and (self.early_stop_main>0):
                fprint(1,self.message_list, f"Early stop at epoch {epoch + 1}, with validation loss: {best_validation}")
                break
        self.evaluate(Tr_x, Tr_y, sample_weight, main_effect_training=True, interaction_training=False)
        self.center_main_effects()
        self.err_val_main_effect_training=np.array(self.err_val_main_effect_training)
    ...
    def prune_main_effect(self, val_x, val_y, sample_weight=None):
        ...
        self.main_effect_val_loss.append(self.evaluate(val_x, val_y, sample_weight, main_effect_training=False, interaction_training=False))
        for idx in range(self.input_num):
            ...
            val_loss = self.evaluate(val_x, val_y, sample_weight, main_effect_training=False, interaction_training=False)
    ...
    def fit_interaction(self, tr_x, tr_y, val_x, val_y, sample_weight=None):
        ...
        for epoch in range(self.interaction_epochs):
            ...
            if self.err_val_interaction_training[-1] < best_validation:
                ...
                last_improvement = epoch
            if epoch - last_improvement > self.early_stop_interaction:
                if self.verbose:
    ...
    def fine_tune_all(self, tr_x, tr_y, val_x, val_y, sample_weight=None):
        ...
        for epoch in range(self.tuning_epochs):
            ...
            if self.err_val_tuning[-1] < best_validation:
                ...
                last_improvement = epoch
            if epoch - last_improvement > self.early_stop_fine:
                if self.verbose:
    ...
    def Init_Fit(self, train_x, train_y, sample_weight):
        ...
        # if self.task_type == "Regression":
        #     tr_x, val_x, tr_y, val_y, tr_idx, val_idx = train_test_split(train_x, train_y, indices, test_size=self.val_ratio, random_state=self.random_state)
        # elif self.task_type == "Classification":
        #     tr_x, val_x, tr_y, val_y, tr_idx, val_idx = train_test_split(train_x, train_y, indices, test_size=self.val_ratio, stratify=train_y, random_state=self.random_state)
        # self.tr_idx = tr_idx
        # self.val_idx = val_idx
        self.estimate_density(train_x, sample_weight)
        # return tr_x, val_x, tr_y, val_y

    def fit(self, train_x, train_y,  sample_weight=None, VAL_X=[], VAL_Y=[], VAL_W=[]):
        ...
        self.Init_Fit(train_x, train_y, sample_weight)
        ...
        if self.training_main:
            ...
            self.Fit_Main_Effect(train_x, train_y, sample_weight, VAL_X, VAL_Y, VAL_W)            
            fprint(1,self.message_list, "#" * 10 + "Stage 1: main effect training stop." + "#" * 10)
            self.prune_main_effect(VAL_X[0], VAL_Y[0], VAL_W[0])
...
class NumerNet(tf.keras.layers.Layer):
    def __init__(self, subnet_arch, subnet_id, activation_func = tf.nn.relu):
...
class NumerNetRNN_Dense(tf.keras.layers.Layer):
    def __init__(self, rnn_units, dense_arch, subnet_id, sequence_length=5, activation_func=tf.nn.relu):
        """
        NumerNet adapted for RNNs.
        
        Args:
        - rnn_units: Number of units in the recurrent layer.
        - dense_arch: List specifying the architecture of the dense layers.
        - activation_func: Activation function for the dense layers.
        - subnet_id: Unique identifier for the subnet.
        """
        super(NumerNetRNN_Dense, self).__init__()
        self.rnn_layer = layers.SimpleRNN(rnn_units, return_sequences=True, return_state=False)
        self.dense_layers = []
        ...
        for nodes in self.dense_arch:
            self.dense_layers.append(layers.Dense(nodes, activation=self.activation_func, kernel_initializer=tf.keras.initializers.Orthogonal()))
        self.output_layer = layers.Dense(1, activation=None, kernel_initializer=tf.keras.initializers.Orthogonal())
    ...
    def call(self, inputs, sample_weight=None, training=False):
        """
        Forward pass for NumerNetRNN.        
        Args:
        - inputs: Input tensor of shape [batch_size, time_steps, features].
        - sample_weight: Optional sample weights for weighted moments.
        - training: Whether the layer is in training mode.
        """
        
        
        # Update min and max values during training
        if training:
            self.min_value.assign(tf.minimum(self.min_value, tf.reduce_min(inputs)))
            self.max_value.assign(tf.maximum(self.max_value, tf.reduce_max(inputs)))

        # Clip inputs based on tracked min and max
        # x = tf.clip_by_value(inputs, self.min_value, self.max_value)
        
        num_samples= len(inputs)
        
        sequence=tf.stack([inputs[i:i + self.sequence_length] for i in range(num_samples - self.sequence_length)])
        
        inputs_0=np.array([list(inputs[i:i + self.sequence_length]) for i in range(num_samples - self.sequence_length)]) 
        inputs_0=inputs_0.reshape(num_samples - self.sequence_length,self.sequence_length,1) 
        print(inputs_0.shape)      
        ...
        # Update moving statistics during training
        if training:
            if sample_weight is None:
                if inputs.shape[0] is not None:
                    sample_weight = tf.ones([inputs.shape[0], 1])
                    self.subnet_mean, self.subnet_norm = tf.nn.weighted_moments(self.output_original, frequency_weights=sample_weight, axes=[0, 1])
            else:
                sample_weight = tf.reshape(sample_weight, shape=(-1, 1))
                self.subnet_mean, self.subnet_norm = tf.nn.weighted_moments(self.output_original, frequency_weights=sample_weight, axes=[0, 1])
        ...
        # Return output (normalized or raw)
        output = self.output_original
        return output
 
class NumerNetRNN0(tf.keras.layers.Layer):
    def __init__(self, rnn_units, dense_arch, activation_func, subnet_id):
        """
        NumerNet adapted for RNNs.
        ...
        - subnet_id: Unique identifier for the subnet.
        """
        super(NumerNetRNN, self).__init__()
        self.rnn_layer = layers.SimpleRNN(rnn_units, return_sequences=True, return_state=False)
        self.dense_layers = []
        self.dense_arch = dense_arch
        self.activation_func = activation_func
